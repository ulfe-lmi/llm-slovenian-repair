# Work order 039-a — Repair adapter integration without recursion
...
{
  "id": "039-a",
  "title": "Repair adapter integration without recursion",
  "objective": "039",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "MVP",
  ...
  "branch": "oap/039-repair-adapter-integration-without-recursion",
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §§11.1–11.2; M06"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§4,5,13"
    }
  ],
...
## Identity
039-a, numeric objective 039, MVP. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §§11.1–11.2; M06. A: ARCHITECTURE.md §§4,5,13.
...
## Governance
Relevant invariants: LR-001, LR-005, LR-006, LR-011. Read C-READ-01 compact sequence,
...
Deliver repair adapter integration without recursion as one separately reviewable seam: Compose completed main response with library and direct private reviewer; no constitutional-context path for review.
Prerequisites: 020, 037, 038.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: api.py main/library/private-review composition. Wire supported complete main output to library then one final delivery; use trusted direct private reviewer independent of main constitutional/gateway route.
...
## Non-goals
No editing neighboring adapters/gateway, bypassing public auth or introducing another model. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect api.py main/library/private-review composition; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Wire supported complete main output to library then one final delivery; use trusted direct private reviewer independent of main constitutional/gateway route.
2. Compose completed main response with library and direct private reviewer; no constitutional-context path for review.
3. Preserve LR-001, LR-005, LR-006, LR-011 across the edge cases below. Bound resources and
...
2. Trace exact fake main and review calls through actual adapter entry point; assert no recursion, no review before capture and no call on absent suspicion.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_039.py -q`.
...
Negative and boundary proof: Trace exact fake main and review calls through actual adapter entry point; assert no recursion, no review before capture and no call on absent suspicion.
