# Work order 025-a — Correct-text negative controls
...
{
  "id": "025-a",
  "title": "Correct-text negative controls",
  "objective": "025",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "MVP",
  ...
  "branch": "oap/025-correct-text-negative-controls",
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §§14.2–14.3; M01"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§7,16.3"
    }
  ],
...
## Identity
025-a, numeric objective 025, MVP. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §§14.2–14.3; M01. A: ARCHITECTURE.md §§7,16.3.
...
## Governance
Relevant invariants: LR-007, LR-009, LR-014. Read C-READ-01 compact sequence,
...
Deliver correct-text negative controls as one separately reviewable seam: Include rare legitimate language, names, technical language and protected content with explicit human review status.
Prerequisites: 023, 024.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: correct-text control selection. Include legitimate rare words, technical vocabulary, proper names, quotations and protected spans; record human correctness review without inventing consensus.
...
## Non-goals
No using Qwen as sole final judge of its own correctness or cherry-picking only easy controls. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect correct-text control selection; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Include legitimate rare words, technical vocabulary, proper names, quotations and protected spans; record human correctness review without inventing consensus.
2. Include rare legitimate language, names, technical language and protected content with explicit human review status.
3. Preserve LR-007, LR-009, LR-014 across the edge cases below. Bound resources and
...
2. Validate source permission and label status; ensure controls challenge unnecessary style/harmful edits and preserve pending review as unproven denominator.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_025.py -q`.
...
Negative and boundary proof: Validate source permission and label status; ensure controls challenge unnecessary style/harmful edits and preserve pending review as unproven denominator.
