...
```oap-metadata
{
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §§6.1–6.3; D02"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§8.2,15"
    }
  ],
...
## Identity
005-a, numeric objective 005, DEMONSTRATOR. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §§6.1–6.3; D02. A: ARCHITECTURE.md §§8.2,15.
...
Deliver real source inventory and permitted acquisition recipe as one separately reviewable seam: Verify actual release/format/rights and a bounded download/import plan; no scraping or invented access.
Prerequisites: 004.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: docs/DATA-SOURCES.md and bounded acquisition recipe. Inspect actual selected resource release pages/formats and terms; distinguish public query access, downloadable lists and permitted redistribution; record unresolved rights.
...
## Non-goals
No bulk scraping, large download, invented license, GPU job or lowering acceptance to hide unavailable data. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect docs/DATA-SOURCES.md and bounded acquisition recipe; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Inspect actual selected resource release pages/formats and terms; distinguish public query access, downloadable lists and permitted redistribution; record unresolved rights.
2. Verify actual release/format/rights and a bounded download/import plan; no scraping or invented access.
...
2. Demonstrate a bounded permitted sample plan with expected checksum/size and cutoff interpretation; document blocked sources with a permitted alternative when available.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_005.py -q`.
...
Negative and boundary proof: Demonstrate a bounded permitted sample plan with expected checksum/size and cutoff interpretation; document blocked sources with a permitted alternative when available.
