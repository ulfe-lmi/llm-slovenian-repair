# Work order 052-a — Explicitly gated candidate deployment/rollback rehearsal
...
{
  "id": "052-a",
  "title": "Explicitly gated candidate deployment/rollback rehearsal",
  "objective": "052",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "SERVICE",
  ...
  "branch": "oap/052-explicitly-gated-candidate-deployment-rollback-rehearsal",
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §§11–13; S04"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§14,15,16"
    }
  ],
...
## Identity
052-a, numeric objective 052, SERVICE. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §§11–13; S04. A: ARCHITECTURE.md §§14,15,16.
...
## Governance
Relevant invariants: LR-011, LR-012, LR-014. Read C-READ-01 compact sequence,
...
Deliver explicitly gated candidate deployment/rollback rehearsal as one separately reviewable seam: Operate only on verified free loopback resources under separate authorization; otherwise prepare/test rehearsal without crossing live gates.
Prerequisites: 047, 049, 050, 051.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: candidate deployment/rollback rehearsal. Prepare finite candidate config, loopback/free-resource verification, disable and rollback steps; perform real rehearsal only under separate explicit authority and applicable DHA clearance.
...
## Non-goals
No occupying existing port, public bind, production cutover, firewall change or automatic startup. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect candidate deployment/rollback rehearsal; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Prepare finite candidate config, loopback/free-resource verification, disable and rollback steps; perform real rehearsal only under separate explicit authority and applicable DHA clearance.
2. Operate only on verified free loopback resources under separate authorization; otherwise prepare/test rehearsal without crossing live gates.
3. Preserve LR-011, LR-012, LR-014 across the edge cases below. Bound resources and
...
2. Disposable fake service exercises start/health/disable/rollback without Qwen mutation; unresolved real port/live approval blocks only actual deployment.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_052.py -q`.
...
Negative and boundary proof: Disposable fake service exercises start/health/disable/rollback without Qwen mutation; unresolved real port/live approval blocks only actual deployment.
