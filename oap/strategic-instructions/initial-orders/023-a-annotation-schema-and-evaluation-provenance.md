# Work order 023-a — Annotation schema and evaluation provenance
...
{
  "id": "023-a",
  "title": "Annotation schema and evaluation provenance",
  "objective": "023",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "MVP",
  ...
  "branch": "oap/023-annotation-schema-and-evaluation-provenance",
...
## Identity
023-a, numeric objective 023, MVP. CREATE_NEW_PR after reconciliation;
...
Deliver annotation schema and evaluation provenance as one separately reviewable seam: Define genuine error, acceptable alternative, style-only and wider-edit labels; permissions and configuration provenance.
Prerequisites: 003, 021.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: annotation schema and evaluation manifest. Define error, acceptable alternatives, style-only and wider-edit labels with explicit human review status, source model settings and acquisition permission.
...
## Non-goals
No raw customer text collection, generating human labels or declaring quality from schema correctness. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect annotation schema and evaluation manifest; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Define error, acceptable alternatives, style-only and wider-edit labels with explicit human review status, source model settings and acquisition permission.
2. Define genuine error, acceptable alternative, style-only and wider-edit labels; permissions and configuration provenance.
...
2. Validate annotated and pending examples; reject ambiguous label state, absent provenance and fixtures pretending synthetic text came from real Qwen.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_023.py -q`.
...
Negative and boundary proof: Validate annotated and pending examples; reject ambiguous label state, absent provenance and fixtures pretending synthetic text came from real Qwen.
