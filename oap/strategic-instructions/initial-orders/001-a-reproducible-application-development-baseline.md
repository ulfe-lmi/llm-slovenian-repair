...
```oap-metadata
{
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §12; D01"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§4.1,15,16"
    }
  ],
...
## Identity
001-a, numeric objective 001, DEMONSTRATOR. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §12; D01. A: ARCHITECTURE.md §§4.1,15,16.
...
## Governance
Relevant invariants: LR-012, LR-014. Read C-READ-01 compact sequence,
...
Deliver reproducible application development baseline as one separately reviewable seam: Establish selected Python/uv packaging, lint/type/test commands and offline default dependencies; no corpus/model download at import.
Prerequisites: 000.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: pyproject.toml, lockfile, offline test entry points. Select compatible Python and packaging versions after local inspection; separate runtime, development and optional CPU-analyzer dependencies.
...
## Non-goals
No model/corpus downloads, API service or production package publication. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect pyproject.toml, lockfile, offline test entry points; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Select compatible Python and packaging versions after local inspection; separate runtime, development and optional CPU-analyzer dependencies.
2. Establish selected Python/uv packaging, lint/type/test commands and offline default dependencies; no corpus/model download at import.
3. Preserve LR-012, LR-014 across the edge cases below. Bound resources and
...
2. Build and install in an owned clean environment; import with outbound network blocked and no corpus present; reject unpinned incompatible constraints.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_001.py -q`.
...
Negative and boundary proof: Build and install in an owned clean environment; import with outbound network blocked and no corpus present; reject unpinned incompatible constraints.
