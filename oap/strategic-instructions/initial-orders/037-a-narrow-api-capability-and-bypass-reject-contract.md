# Work order 037-a — Narrow API capability and bypass/reject contract
...
{
  "id": "037-a",
  "title": "Narrow API capability and bypass/reject contract",
  "objective": "037",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "MVP",
  ...
  "branch": "oap/037-narrow-api-capability-and-bypass-reject-contract",
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §§11.3–11.4; M06"
    },
    {
      "kind": "A",
...
## Identity
037-a, numeric objective 037, MVP. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §§11.3–11.4; M06. A: ARCHITECTURE.md §§13.
...
## Governance
Relevant invariants: LR-001, LR-005, LR-011, LR-014. Read C-READ-01 compact sequence,
...
Deliver narrow api capability and bypass/reject contract as one separately reviewable seam: Select one verified nonstreaming text envelope; explicit policy for tools/JSON/SSE/stateful/multiple-choice/logprobs cases.
Prerequisites: 020, 036.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: API capability contract. Select one verified endpoint/envelope and one nonstreaming text choice; state pre-main rejection versus configured unchanged bypass for each unsupported capability.
...
## Non-goals
No universal Chat/Responses compatibility, JSON under SSE contract or public bypass header. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect API capability contract; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Select one verified endpoint/envelope and one nonstreaming text choice; state pre-main rejection versus configured unchanged bypass for each unsupported capability.
2. Select one verified nonstreaming text envelope; explicit policy for tools/JSON/SSE/stateful/multiple-choice/logprobs cases.
3. Preserve LR-001, LR-005, LR-011, LR-014 across the edge cases below. Bound resources and
...
2. Contract table/tests cover tools, JSON schema, SSE, stateful links, multiple choices, logprobs and offset annotations; unsupported requests must not silently reach wrong mode.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_037.py -q`.
...
Negative and boundary proof: Contract table/tests cover tools, JSON schema, SSE, stateful links, multiple choices, logprobs and offset annotations; unsupported requests must not silently reach wrong mode.
