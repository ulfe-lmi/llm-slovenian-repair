# Work order 007-a — Bigram/trigram importer with censored absence
...
{
  "id": "007-a",
  "title": "Bigram/trigram importer with censored absence",
  "objective": "007",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "DEMONSTRATOR",
  ...
  "branch": "oap/007-bigram-trigram-importer-with-censored-absence",
...
## Identity
007-a, numeric objective 007, DEMONSTRATOR. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §6.2; D02. A: ARCHITECTURE.md §§8.1,8.2.
...
Deliver bigram/trigram importer with censored absence as one separately reviewable seam: Test threshold interpretation, malformed data and impossible-zero rejection.
Prerequisites: 004, 005, 006.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: bigram/trigram importer. Model retained counts and absent censored bounds separately; carry release-specific threshold units rather than a universal hardcoded Gigafida cutoff.
...
## Non-goals
No frequency-derived semantic correctness claim or unlicensed bulk acquisition. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect bigram/trigram importer; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Model retained counts and absent censored bounds separately; carry release-specific threshold units rather than a universal hardcoded Gigafida cutoff.
2. Test threshold interpretation, malformed data and impossible-zero rejection.
...
2. Assert threshold edge cases and missing rows remain CENSORED; reject fabricated complete denominators, invalid count units and malformed n-gram widths.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_007.py -q`.
...
Negative and boundary proof: Assert threshold edge cases and missing rows remain CENSORED; reject fabricated complete denominators, invalid count units and malformed n-gram widths.
