# Work order 009-a — Unicode token/span mapping
...
{
  "id": "009-a",
  "title": "Unicode token/span mapping",
  "objective": "009",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "DEMONSTRATOR",
  ...
  "branch": "oap/009-unicode-token-span-mapping",
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §5.2; D03"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§6,7"
    }
  ],
...
## Identity
009-a, numeric objective 009, DEMONSTRATOR. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §5.2; D03. A: ARCHITECTURE.md §§6,7.
...
## Governance
Relevant invariants: LR-003, LR-007. Read C-READ-01 compact sequence,
...
Deliver unicode token/span mapping as one separately reviewable seam: Exact substrings for repeated words, combining characters, emoji and CRLF; no detokenization rewrite.
Prerequisites: 003.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: tokenizer.py original-coordinate mapping. Keep the exact Python input string immutable and derive analytic tokens with reversible offsets; distinguish repeated identical occurrences by position.
...
## Non-goals
No detokenization reconstruction, HTTP offset conversion or normalization of original answer. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect tokenizer.py original-coordinate mapping; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Keep the exact Python input string immutable and derive analytic tokens with reversible offsets; distinguish repeated identical occurrences by position.
2. Exact substrings for repeated words, combining characters, emoji and CRLF; no detokenization rewrite.
3. Preserve LR-003, LR-007 across the edge cases below. Bound resources and
...
2. Check č/š/ž, composed/decomposed marks, emoji, repeated words and CRLF; assert every slice equality and abstain from unsafe partial-grapheme boundaries.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_009.py -q`.
...
Negative and boundary proof: Check č/š/ž, composed/decomposed marks, emoji, repeated words and CRLF; assert every slice equality and abstain from unsafe partial-grapheme boundaries.
