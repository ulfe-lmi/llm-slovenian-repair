# Work order 050-a — Health/readiness and disable/rollback behavior
...
{
  "id": "050-a",
  "title": "Health/readiness and disable/rollback behavior",
  "objective": "050",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "SERVICE",
  ...
  "branch": "oap/050-health-readiness-and-disable-rollback-behavior",
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §§13.3, 15.3; S04"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§14.2,15"
    }
  ],
...
## Identity
050-a, numeric objective 050, SERVICE. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §§13.3, 15.3; S04. A: ARCHITECTURE.md §§14.2,15.
...
## Governance
Relevant invariants: LR-002, LR-011, LR-012. Read C-READ-01 compact sequence,
...
Deliver health/readiness and disable/rollback behavior as one separately reviewable seam: Reflect index/config availability honestly; disable optional repair without model replacement; test degraded originals and hard errors separately.
Prerequisites: 036, 041, 049.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: health/readiness and repair disable controls. Expose honest config/index availability and controlled disable path preserving completed originals; model replacement is unnecessary to disable optional repair.
...
## Non-goals
No suppressing gateway validation/auth errors or restarting protected Qwen as recovery. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect health/readiness and repair disable controls; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Expose honest config/index availability and controlled disable path preserving completed originals; model replacement is unnecessary to disable optional repair.
2. Reflect index/config availability honestly; disable optional repair without model replacement; test degraded originals and hard errors separately.
3. Preserve LR-002, LR-011, LR-012 across the edge cases below. Bound resources and
...
2. Test missing/corrupt index, disabled repair zero calls, queue failure and hard capture/main error separation; readiness must not claim quality evidence.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_050.py -q`.
...
Negative and boundary proof: Test missing/corrupt index, disabled repair zero calls, queue failure and hard capture/main error separation; readiness must not claim quality evidence.
