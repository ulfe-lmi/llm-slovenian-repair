# Work order 041-a — Envelope preservation, usage and private metrics
...
{
  "id": "041-a",
  "title": "Envelope preservation, usage and private metrics",
  "objective": "041",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "MVP",
  ...
  "branch": "oap/041-envelope-preservation-usage-and-private-metrics",
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §§10, 11.4, 13.4; M06"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§12,13,15"
    }
  ],
...
## Identity
041-a, numeric objective 041, MVP. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §§10, 11.4, 13.4; M06. A: ARCHITECTURE.md §§12,13,15.
...
Deliver envelope preservation, usage and private metrics as one separately reviewable seam: Separate main/review usage and fixed-label timings; no content logs or falsely aligned logprobs; no-edit preservation tested.
Prerequisites: 039, 040.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: envelope/usage and private metrics. Transform only supported final text field; preserve pass-through envelope when possible and account main generation separately from review inference.
...
## Non-goals
No fabricated token probabilities, counting final text as actual bill or logging private content. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect envelope/usage and private metrics; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Transform only supported final text field; preserve pass-through envelope when possible and account main generation separately from review inference.
2. Separate main/review usage and fixed-label timings; no content logs or falsely aligned logprobs; no-edit preservation tested.
...
2. Test unchanged/changed paths, finish reasons, usage and stale logprob/annotation capabilities; secret/text canaries must be absent from fixed-label metrics and errors.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_041.py -q`.
...
Negative and boundary proof: Test unchanged/changed paths, finish reasons, usage and stale logprob/annotation capabilities; secret/text canaries must be absent from fixed-label metrics and errors.
