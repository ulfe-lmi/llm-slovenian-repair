...
```oap-metadata
{
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §§1, 12, 16"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§1,17,20"
    }
  ],
...
## Identity
000-a, numeric objective 000, DEMONSTRATOR. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §§1, 12, 16. A: ARCHITECTURE.md §§1,17,20.
...
Deliver reconcile bootstrap, repository and authority as one separately reviewable seam: Adopt the reviewed scaffold baseline on the first product PR; prove role routing/dense projections, source identity, inactive-to-active protocol and no implementation overclaim.
Prerequisites: owner-reviewed, deliberately published scaffold baseline.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: oap/bin, role projections and owner-reviewed baseline. Compare exact canonical source bytes and transitive coding reads; reconcile accepted remote baseline and existing local changes before adopting the scaffold.
...
## Non-goals
Do not regenerate accepted bootstrap, choose a remote, infer auth, or implement repair code. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect oap/bin, role projections and owner-reviewed baseline; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Compare exact canonical source bytes and transitive coding reads; reconcile accepted remote baseline and existing local changes before adopting the scaffold.
2. Adopt the reviewed scaffold baseline on the first product PR; prove role routing/dense projections, source identity, inactive-to-active protocol and no implementation overclaim.
...
2. Prove absent versus malformed active and strict rejection of draft metadata using disposable repositories; capture final-head governance checks.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_000.py -q`.
...
Negative and boundary proof: Prove absent versus malformed active and strict rejection of draft metadata using disposable repositories; capture final-head governance checks.
