# Work order 032-a — Versioned strict acceptance thresholds
...
{
  "id": "032-a",
  "title": "Versioned strict acceptance thresholds",
  "objective": "032",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "MVP",
  ...
  "branch": "oap/032-versioned-strict-acceptance-thresholds",
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §9; M04"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§11,16.3"
    }
  ],
...
## Identity
032-a, numeric objective 032, MVP. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §9; M04. A: ARCHITECTURE.md §§11,16.3.
...
## Governance
Relevant invariants: LR-009, LR-014. Read C-READ-01 compact sequence,
...
Deliver versioned strict acceptance thresholds as one separately reviewable seam: Tune on calibration data only, by error class; leave experimental acceptance disabled; document coverage tradeoff.
Prerequisites: 026, 028, 029, 030, 031.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: versioned error-class strict thresholds. Calibrate finite acceptance policy by error class with compatible independent evidence; record reasons, quality/coverage tradeoffs and immutable policy version.
...
## Non-goals
Do not promise 99% precision before measurement or relax agreed independent-evidence boundary. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect versioned error-class strict thresholds; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Calibrate finite acceptance policy by error class with compatible independent evidence; record reasons, quality/coverage tradeoffs and immutable policy version.
2. Tune on calibration data only, by error class; leave experimental acceptance disabled; document coverage tradeoff.
3. Preserve LR-009, LR-014 across the edge cases below. Bound resources and
...
2. Replay calibration sets and controls; reject confidence-only/neutral evidence; prove held-out data absent from tuning inputs and experimental mode remains disabled.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_032.py -q`.
...
Negative and boundary proof: Replay calibration sets and controls; reject confidence-only/neutral evidence; prove held-out data absent from tuning inputs and experimental mode remains disabled.
