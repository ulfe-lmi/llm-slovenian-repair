# Work order 026-a — Frozen calibration/test split and dataset integrity
...
{
  "id": "026-a",
  "title": "Frozen calibration/test split and dataset integrity",
  "objective": "026",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "MVP",
  ...
  "branch": "oap/026-frozen-calibration-test-split-and-dataset-integrity",
...
## Identity
026-a, numeric objective 026, MVP. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §14.2; M01. A: ARCHITECTURE.md §§16.3.
...
Deliver frozen calibration/test split and dataset integrity as one separately reviewable seam: Prevent cross-split duplicates/leakage; version inputs and annotation status; distinguish pending human labels.
Prerequisites: 023, 024, 025.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: dataset split and integrity tooling. Freeze calibration and held-out partitions with provenance/content digests and duplicate detection; separate human-label revisions from tuning configuration.
...
## Non-goals
No inspecting held-out outcomes to tune thresholds or silently moving difficult examples. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect dataset split and integrity tooling; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Freeze calibration and held-out partitions with provenance/content digests and duplicate detection; separate human-label revisions from tuning configuration.
2. Prevent cross-split duplicates/leakage; version inputs and annotation status; distinguish pending human labels.
...
2. Reject exact/near-duplicate leakage and held-out mutation; rerun reproduces membership; unresolved labels remain visible and excluded from claimed evaluated counts.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_026.py -q`.
...
Negative and boundary proof: Reject exact/near-duplicate leakage and held-out mutation; rerun reproduces membership; unresolved labels remain visible and excluded from claimed evaluated counts.
