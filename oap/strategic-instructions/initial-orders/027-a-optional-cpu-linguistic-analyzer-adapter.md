# Work order 027-a — Optional CPU linguistic-analyzer adapter
...
{
  "id": "027-a",
  "title": "Optional CPU linguistic-analyzer adapter",
  "objective": "027",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "MVP",
  ...
  "branch": "oap/027-optional-cpu-linguistic-analyzer-adapter",
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §§6.1, 12; M02"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§4.1,8,9"
    }
  ],
...
## Identity
027-a, numeric objective 027, MVP. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §§6.1, 12; M02. A: ARCHITECTURE.md §§4.1,8,9.
...
## Governance
Relevant invariants: LR-008, LR-012, LR-014. Read C-READ-01 compact sequence,
...
Deliver optional cpu linguistic-analyzer adapter as one separately reviewable seam: Integrate verified existing lemma/tagger capability behind an optional bounded interface; no second GPU model.
Prerequisites: 003, 005, 026.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: optional morphology analyzer adapter. Qualify an existing CPU analyzer behind injected finite interface and compatible tag/normalization manifest; document memory/load and ambiguous output.
...
## Non-goals
No second GPU model, import-time download, altered Qwen environment or removal of morphology requirement. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect optional morphology analyzer adapter; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Qualify an existing CPU analyzer behind injected finite interface and compatible tag/normalization manifest; document memory/load and ambiguous output.
2. Integrate verified existing lemma/tagger capability behind an optional bounded interface; no second GPU model.
3. Preserve LR-008, LR-012, LR-014 across the edge cases below. Bound resources and
...
2. Fake adapter tests missing dependency, timeout, uncertainty and concurrent limits; optional nonadoption needs measured rationale while required MVP morphology behavior remains planned.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_027.py -q`.
...
Negative and boundary proof: Fake adapter tests missing dependency, timeout, uncertainty and concurrent limits; optional nonadoption needs measured rationale while required MVP morphology behavior remains planned.
