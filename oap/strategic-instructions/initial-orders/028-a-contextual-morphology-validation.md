# Work order 028-a — Contextual morphology validation
...
{
  "id": "028-a",
  "title": "Contextual morphology validation",
  "objective": "028",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "MVP",
  ...
  "branch": "oap/028-contextual-morphology-validation",
...
## Identity
028-a, numeric objective 028, MVP. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §7.3; M02. A: ARCHITECTURE.md §§9,11.2.
...
Deliver contextual morphology validation as one separately reviewable seam: Judge required case/number/gender from context, not erroneous original tags; ambiguity causes abstention.
Prerequisites: 017, 027.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: contextual morphology validation. Derive required case/number/gender from sentence context and confidence; distinguish lexical replacement from correcting an erroneous original inflection.
...
## Non-goals
No broad grammar rewriting, hardcoded Slovenian example replacements or analyzer confidence as human accuracy. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect contextual morphology validation; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Derive required case/number/gender from sentence context and confidence; distinguish lexical replacement from correcting an erroneous original inflection.
2. Judge required case/number/gender from context, not erroneous original tags; ambiguity causes abstention.
...
2. Test mismatched originals, valid contextual alternatives, ambiguous lemmas and insufficient tags; do not require replacement to copy incorrect original tags.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_028.py -q`.
...
Negative and boundary proof: Test mismatched originals, valid contextual alternatives, ambiguous lemmas and insufficient tags; do not require replacement to copy incorrect original tags.
