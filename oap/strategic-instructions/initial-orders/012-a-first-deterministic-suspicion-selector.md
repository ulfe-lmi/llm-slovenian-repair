# Work order 012-a — First deterministic suspicion selector
...
{
  "id": "012-a",
  "title": "First deterministic suspicion selector",
  "objective": "012",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "DEMONSTRATOR",
  ...
  "branch": "oap/012-first-deterministic-suspicion-selector",
...
## Identity
012-a, numeric objective 012, DEMONSTRATOR. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §7; D04. A: ARCHITECTURE.md §§9.
...
## Governance
Relevant invariants: LR-002, LR-003, LR-008, LR-010. Read C-READ-01 compact sequence,
...
Deliver first deterministic suspicion selector as one separately reviewable seam: Explain alarms using source evidence, choose bounded non-overlapping word/phrase spans, support zero-alarm path.
Prerequisites: 008, 011.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: detector.py deterministic target selector. Combine lexicon/unigram/neighbor signals with explicit source confidence; group only bounded original spans and order selected targets deterministically.
...
## Non-goals
Do not correct text, hardcode motivating examples or turn rare words into proven errors. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect detector.py deterministic target selector; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Combine lexicon/unigram/neighbor signals with explicit source confidence; group only bounded original spans and order selected targets deterministically.
2. Explain alarms using source evidence, choose bounded non-overlapping word/phrase spans, support zero-alarm path.
3. Preserve LR-002, LR-003, LR-008, LR-010 across the edge cases below. Bound resources and
...
2. Exercise no alarms, one anomaly, competing/overlapping alarms, weak surrounding evidence and target-budget overflow; verify zero reviewer calls on no eligible target.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_012.py -q`.
...
Negative and boundary proof: Exercise no alarms, one anomaly, competing/overlapping alarms, weak surrounding evidence and target-budget overflow; verify zero reviewer calls on no eligible target.
