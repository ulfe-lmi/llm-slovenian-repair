# Work order 048-a — Broader negative/harm evaluation
...
{
  "id": "048-a",
  "title": "Broader negative/harm evaluation",
  "objective": "048",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "SERVICE",
  ...
  "branch": "oap/048-broader-negative-harm-evaluation",
...
## Identity
048-a, numeric objective 048, SERVICE. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §14.3; S01. A: ARCHITECTURE.md §§16.3.
...
## Governance
Relevant invariants: LR-009, LR-013, LR-014. Read C-READ-01 compact sequence,
...
Deliver broader negative/harm evaluation as one separately reviewable seam: Run defined approved held-out batches, quantify uncertainty and clustered-data limitations; human annotation time explicit.
Prerequisites: 025, 026, 034, 044.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: broader harm/negative evaluation. Evaluate approved held-out correct/technical/rare domains with human annotation time and clustered sample caveats; freeze configurations and include failed cases.
...
## Non-goals
No tuning held-out results, hiding failures or manufacturing statistically sufficient sample counts. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect broader harm/negative evaluation; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Evaluate approved held-out correct/technical/rare domains with human annotation time and clustered sample caveats; freeze configurations and include failed cases.
2. Run defined approved held-out batches, quantify uncertainty and clustered-data limitations; human annotation time explicit.
3. Preserve LR-009, LR-013, LR-014 across the edge cases below. Bound resources and
...
2. Aggregate useful corrections, harmful and style-only edits with denominators/uncertainty; test no-event upper-bound interpretation and pending-label exclusions.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_048.py -q`.
...
Negative and boundary proof: Aggregate useful corrections, harmful and style-only edits with denominators/uncertainty; test no-event upper-bound interpretation and pending-label exclusions.
