# Work order 019-a — Exact patcher and conflict-group rollback
...
{
  "id": "019-a",
  "title": "Exact patcher and conflict-group rollback",
  "objective": "019",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "DEMONSTRATOR",
  ...
  "branch": "oap/019-exact-patcher-and-conflict-group-rollback",
...
## Identity
019-a, numeric objective 019, DEMONSTRATOR. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §10; D06. A: ARCHITECTURE.md §§12.
...
## Governance
Relevant invariants: LR-006, LR-007, LR-009. Read C-READ-01 compact sequence,
...
Deliver exact patcher and conflict-group rollback as one separately reviewable seam: Preserve every untouched original slice; reject overlap/stale offsets; rollback incompatible nearby edits.
Prerequisites: 009, 010, 017, 018.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: patcher.py exact composition. Recheck original slices and compose untouched segments plus approved replacements; reject overlap and incompatible neighboring edits as conflict groups.
...
## Non-goals
No last-wins conflict handling, Markdown renderer, model-regenerated answer or second generative pass. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect patcher.py exact composition; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Recheck original slices and compose untouched segments plus approved replacements; reject overlap and incompatible neighboring edits as conflict groups.
2. Preserve every untouched original slice; reject overlap/stale offsets; rollback incompatible nearby edits.
3. Preserve LR-006, LR-007, LR-009 across the edge cases below. Bound resources and
...
2. Use length-changing edits in multiple positions and stale offsets; assert exact untouched slices and original equality when every group is rejected.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_019.py -q`.
...
Negative and boundary proof: Use length-changing edits in multiple positions and stale offsets; assert exact untouched slices and original equality when every group is rejected.
