# Work order 047-a — Measured latency and resource budget
...
{
  "id": "047-a",
  "title": "Measured latency and resource budget",
  "objective": "047",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "SERVICE",
  ...
  "branch": "oap/047-measured-latency-and-resource-budget",
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §13.2; S03"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§14,16"
    }
  ],
...
## Identity
047-a, numeric objective 047, SERVICE. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §13.2; S03. A: ARCHITECTURE.md §§14,16.
...
Deliver measured latency and resource budget as one separately reviewable seam: Separate CPU, queue, review and main timings; report memory and workload limits on verified hardware without reconfiguring Qwen.
Prerequisites: 034, 036, 043.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: latency/resource measurement harness. Measure CPU detection, queue, review, validation, main and delivery separately with actual workload/model/source/policy identities and finite budgets.
...
## Non-goals
No claimed millisecond targets as results or alteration of Qwen/shared GPU allocation to improve benchmark. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect latency/resource measurement harness; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Measure CPU detection, queue, review, validation, main and delivery separately with actual workload/model/source/policy identities and finite budgets.
2. Separate CPU, queue, review and main timings; report memory and workload limits on verified hardware without reconfiguring Qwen.
...
2. Report distributions/sample counts and memory under bounded load; validate clocks/accounting with fakes first; distinguish no-review and review paths.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_047.py -q`.
...
Negative and boundary proof: Report distributions/sample counts and memory under bounded load; validate clocks/accounting with fakes first; distinguish no-review and review paths.
