# Work order 042-a — API failure, disconnect and concurrency tests
...
{
  "id": "042-a",
  "title": "API failure, disconnect and concurrency tests",
  "objective": "042",
  "status": "DRAFT UNTIL STRATEGIC RECONCILIATION",
  "phase": "MVP",
  ...
  "branch": "oap/042-api-failure-disconnect-and-concurrency-tests",
  ...
  "provenance": [
    {
      "kind": "H",
      "reference": "PLAN.md §§13.3, 14.1; M05–M06"
    },
    {
      "kind": "A",
      "reference": "ARCHITECTURE.md §§14,16.1"
    }
  ],
...
## Identity
042-a, numeric objective 042, MVP. CREATE_NEW_PR after reconciliation;
if interrupted after PR creation, adopt that same verified branch/PR.

## Provenance
H: PLAN.md §§13.3, 14.1; M05–M06. A: ARCHITECTURE.md §§14,16.1.
...
## Governance
Relevant invariants: LR-001, LR-005, LR-010, LR-011, LR-013. Read C-READ-01 compact sequence,
...
Deliver api failure, disconnect and concurrency tests as one separately reviewable seam: Cancel owned work, handle timeout/size/invalid shape, preserve tenant request isolation and avoid orphan review tasks.
Prerequisites: 035, 036, 038, 039, 040, 041.
Confirm their actual contracts/evidence, not just existence of earlier draft files.

## Scope
Primary boundary: integrated API negative-path suite. Exercise request ownership and finite resources through actual API entry point with fake upstream outside transport boundary; preserve hard versus optional failures.
...
## Non-goals
No replacing adapter under test with a fake or weakening errors to make integration green. Do not absorb adjacent objectives or change human product intent.

## Files and boundaries
Inspect integrated API negative-path suite; existing interfaces supplied by prerequisites; relevant compact
...
## Requirements
1. Exercise request ownership and finite resources through actual API entry point with fake upstream outside transport boundary; preserve hard versus optional failures.
2. Cancel owned work, handle timeout/size/invalid shape, preserve tenant request isolation and avoid orphan review tasks.
3. Preserve LR-001, LR-005, LR-010, LR-011, LR-013 across the edge cases below. Bound resources and
...
2. Test disconnect before/after capture, queue full, invalid batch/shape, oversize, timeout and concurrent tenants; assert cleanup and no orphan/cross-tenant review.
...
## Verification
Focused future test target: `uv run pytest tests/contract/test_objective_042.py -q`.
...
Negative and boundary proof: Test disconnect before/after capture, queue full, invalid batch/shape, oversize, timeout and concurrent tenants; assert cleanup and no orphan/cross-tenant review.
