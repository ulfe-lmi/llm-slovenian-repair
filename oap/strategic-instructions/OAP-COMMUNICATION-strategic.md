# Full strategic communication — revision 1

Applies only to explicitly selected strategic role. This project-specific operating contract is derived from the supplied protocol 2.0; its generator instructions below identify historical bootstrap duties, not orders to repeat them.
...
## 11. Executable profile and operator commands

All Markdown metadata uses one `oap-metadata` JSON fence for orders, `oap-report`
for reports and `oap-ica` for audits. Use generated templates. Required readable
sections remain normative. Metadata includes exact source identities and current
observations; quoted synthetic fixtures do not become finalization markers.
The helper validates structure and bytes, never authenticates strategic/human
judgment from a label. Runtime configuration is private `runtime.env`, allowlisted
KEY=JSON-string data, never shell-sourced. No raw env/auth output in receipts.

Before publication run doctor and accepted-runtime governance with the literal
trusted accepted Git revision in OAP_ACCEPTED_REF. Reconcile configured GitHub
repository, main and existing objective PR. The accepted reference is deliberately
selected after baseline review; no candidate-authored expected hash grants approval.

Publish with `python3 oap/bin/publish_order.py --repo-root PATH --strategic-home
PATH --accepted-ref SHA --repository OWNER/REPO --source FINAL_FILE --id NNN-a`.
Use --dry-run first; it creates no files or locks. Role must be strategic.
Publication is immutable order then atomic active replacement, without implicit
signal/commit/push/model invocation. Signal separately only after validation with
`python3 oap/bin/oap_fifo.py send --fifo PATH`; response receive uses `wait`.
Use exact configured private FIFO paths, never historical host examples.

On response, run `verify_report.py --repo-root PATH --id ID --repository OWNER/REPO`.
Local-only verification is explicitly labelled and insufficient. gh is a read-only
boundary used to inspect PR identity/head, commit parent/files and report bytes.
Independently inspect final diff/evidence/CI and record a bounded private review.
`strategic_gate.py` checks reviewed SHA, expected checks and development-only effect;
it performs no merge and cannot replace semantic review. Apply repository-approved
merge method with expected reviewed SHA only after full S-MERGE-01 review, then
use `--verify-merge` and verify default branch. Missing check, changed head or
auto-deploy side effect blocks this action. Do not disable repository protection.

Private post-publication receipts record actual observed remote SHA and result,
never raw model outputs. A final report must not retroactively claim future push
or CI. The coding model sends response OK after verification; wrapper verifies
again before permitting another invocation, and remains outside the model while idle.

Recovery is explicit. `check_state.py` reports INACTIVE, READY, RECOVERY_REQUIRED,
PUBLICATION_RECONCILIATION_REQUIRED or REVIEW_READY. Record incident/recovery reason,
reconcile actual branch/PR and unfinished acceptance. For consumed unfinished work,
launch with `--resume-id ID` and deliberately re-signal only that same round.
Final remote report means review, not replay. Local report cannot be rewritten to
hide failed publication. A killed process or user quit never advances scope.

Candidate governance review uses `check_governance.py --mode candidate-review
--accepted-ref SHA --allow-change EXACT_PATH` for every explicitly ordered changed
source/projection. Private law stays accepted-old. After accepted merge and a
reconciled inactive checkpoint, `bootstrap-two-codex-oap.sh --refresh-governance
--accepted-ref SHA` takes role locks, preserves backups and updates only approved
law/references. Reread before another order; drafts/config/history remain private.

Audit: finalize request with exact accepted architecture/PLAN identities and
current merged-main SHA; fresh context derives full-source requirements before
reading strategy claims. Check returned structure with `check_ica.py --source FILE
--current-main SHA`. UNPROVEN rows and declared independence are not semantic
acceptance; verify actual boundaries and repeat after remediation. Human owns label.
