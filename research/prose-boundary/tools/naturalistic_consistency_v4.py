"""008-i requirement 8: naturalistic consistency re-run under the 008-i
implementation (committed deterministic driver; data-free aggregate under
results/hidden-acceptance-v4/).

With the frozen 008-i implementation (the 008-h implementation plus the
single predeclared D1-5 xml-fragment trigger condition, frozen at the dev
regression gate and recorded in experiment-008i.json), re-runs the
protection output on the 70 naturalistic responses (private root, read
through the disclosed adjudication loading only) against the EXISTING
completed v2 labels (frozen adjudication outputs - no re-adjudication except
the narrow predeclared label-correction path of scope item 8, which is
agent-level and, if used, recorded separately in the aggregate with
rationale).
...
Resolution kinds (carried from the 008-h D1-4 registration):
  CLOSED-RESIDUAL                 - covered by a bounded residual
                                    extension/trigger implemented in a
                                    completed round (named categories, strict
                                    trigger, negative controls); class
                                    reference recorded;
  DOCUMENTED-DIALECT-LIMITATION   - a policy class documents the shape
...
- the committed shape analysis.

The 008-i re-run (predeclared expectation: 0 UNRESOLVED, 0 label
corrections): the D1-5 refinement only ADDS protection (the xml-fragment
class fires where it previously abstained on the exact measured
vocabulary-name + non-ASCII-attribute-name condition); no previously
protected byte is unprotected, so every 008-g exposed range either keeps its
resolution kind or shrinks (or disappears) - it cannot appear as a new
unresolved material range. Any exposed range outside the embedded map would
be reported UNRESOLVED (fail-closed).
...
Usage (private roots are invocation arguments; no private paths in this
committed file):
  python3 -B naturalistic_consistency_v4.py \
...
OUT = REPO / "research/prose-boundary/results/hidden-acceptance-v4/naturalistic-consistency-v4.json"
MATERIAL_BYTES = 50
...
# classes. Keyed by (case_id, (start_line, end_line)). Carried unchanged
# into the 008-i re-run (the 008-i D1-5 refinement only adds protection).
...
def main() -> int:
    ...
    out = {
        "schema": "008i-naturalistic-consistency-v4/1",
        "order": "008-i",
        "label": ("naturalistic consistency re-run under the 008-i implementation "
                  "(order requirement 8): the frozen 008-i implementation's "
                  "(008-h implementation + the single predeclared D1-5 "
                  "xml-fragment trigger condition, frozen at the dev "
                  "regression gate; census in experiment-008i.json) "
                  "protection output on the 70 naturalistic responses "
                  "compared against the EXISTING completed v2 labels (fixed "
                  "population, no re-adjudication except the narrow "
                  "predeclared label-correction path); data-free aggregate "
                  "(counts, case IDs, line/byte references only; no response "
                  "content, no private paths)"),
        ...
        "d1_5_monotonicity_note": ("the 008-i D1-5 refinement only adds "
                                   "protection (the xml-fragment class fires "
                                   "where it previously abstained on the exact "
                                   "measured vocabulary-name + non-ASCII-"
                                   "attribute-name condition); no previously "
                                   "protected byte is unprotected, so every "
                                   "008-g exposed range either keeps its "
                                   "resolution kind or shrinks/disappears"),
        ...
        "implementation_identity": {
            ...
            "note": "the frozen 008-i implementation (frozen protection identity recorded in experiment-008i.json)",
        },
    }
