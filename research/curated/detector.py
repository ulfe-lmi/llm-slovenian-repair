...
def _evidence(
    ...
    if index > 0 and _between_is_open(tokens[index - 1], token, intervals):
        result["left_bigram"] = asdict(
            corpus.ngram(f"{tokens[index - 1].key} {token.key}", 2)
        )
    if index + 1 < len(tokens) and _between_is_open(token, tokens[index + 1], intervals):
        result["right_bigram"] = asdict(
            corpus.ngram(f"{token.key} {tokens[index + 1].key}", 2)
        )
    if (
        index > 0
        ...
        result["alternatives"] = corpus.alternatives(
            tokens[index - 1].key, tokens[index + 1].key
        )
    return result
...
def detect(
    ...
    *,
    ...
    """Run the frozen detector; ``maximum=None`` is uncapped campaign mode."""
