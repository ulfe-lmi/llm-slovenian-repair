"""Faithful detector copied from the historical uncapped/hyphen runs.

The detector only selects suspicions. ``maximum=None`` is the uncapped
campaign setting; finite values reproduce the earlier capped variants.
"""
...
def _evidence(corpus: Corpus, tokens: list[Token], index: int, intervals: list[Interval]) -> dict[str, object]:
    token = tokens[index]
    result: dict[str, object] = {"unigram": asdict(corpus.unigram(token.key))}
