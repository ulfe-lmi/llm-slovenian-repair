...
- Variant-specific wire/content boundary: Each scheduled trial uses the symmetric initial-case family and word-only retry; stopped trials continue unchanged. See [configuration](../configs/ten-run-initial-case-low.json) fields historical_wire_keys, historical_content, and model_call_policy; no later variant's content is inferred.
