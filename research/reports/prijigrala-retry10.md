...
- Variant-specific wire/content boundary: The one-target driver sends the same raw first rejected replacement on each corrective retry and stops at ten. See [configuration](../configs/prijigrala-retry10.json) fields historical_wire_keys, historical_content, and model_call_policy; no later variant's content is inferred.
