...
- Status: `STOPPED_WITH_WHITESPACE_CONTINUATION`; an old `RUNNING` marker is not completion.
- Private logical root: `experiments/low-word-only-retry-20260909.0Hk0P9`; native path and payloads are not published.

## Configuration projection

- Variant-specific wire/content boundary: The retry family sends the raw rejected word only; saved whitespace continuation does not resample the first response. See [configuration](../configs/low-word-only-retry.json) fields historical_wire_keys, historical_content, and model_call_policy; no later variant's content is inferred.
