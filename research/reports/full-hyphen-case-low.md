...
- Variant-specific wire/content boundary: The early hyphen family uses the no-English policy, one-way initial-case rule, and word-only retry. See [configuration](../configs/full-hyphen-case-low.json) fields historical_wire_keys, historical_content, and model_call_policy; no later variant's content is inferred.
