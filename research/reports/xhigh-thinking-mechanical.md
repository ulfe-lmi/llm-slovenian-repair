...
- Question: What did the frozen direct reviewer do with xhigh reasoning?
- Authorized change: direct review with xhigh reasoning and mechanical-only acceptance
- Status: `COMPLETED`; an old `RUNNING` marker is not completion.
- Private logical root: `experiments/xhigh-thinking-mechanical-20260909.ZsPBb5`; native path and payloads are not published.

## Configuration projection

- Variant-specific wire/content boundary: The early direct family sends one selected-target xhigh-reasoning review with mechanical-only acceptance. See [configuration](../configs/xhigh-thinking-mechanical.json) fields historical_wire_keys, historical_content, and model_call_policy; no later variant's content is inferred.
...
| `cases` | `32` |
| `changed_controls` | `1` |
| `exact_gold_repairs` | `4` |
| `missed_gold_errors` | `3` |
| `mode` | `reasoning_xhigh` |
| `model_calls` | `10` |
| `proposed_edits` | `8` |
| `qualified_calls` | `10` |
| `retry_calls` | `0` |
...
- [Configuration](../configs/xhigh-thinking-mechanical.json)
